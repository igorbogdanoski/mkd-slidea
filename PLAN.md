...
- [x] **6.1 Curriculum mapping** ✅ — `src/data/mkMathCurriculum.js` (компактна таксономија ~50 тагови за MK математика G1–G9 базирано на `igorbogdanoski/math-curriculum-ai-navigator`); `src/lib/curriculumTagger.js` (pure-JS heuristic `suggestTags()` со cyrillic-aware normalize + бонус за literal subtopic/topic match); `src/components/CurriculumTagPicker.jsx` (chips UI со auto-предлози + grade dropdown + browse list, max 4); вграден во `CreatePollModal` (state `curriculumTags`, hydrate од `initialData.curriculum_tags`, persist во `polls.curriculum_tags TEXT[]`); `SUPABASE_CURRICULUM.sql` додава колоната + GIN index. Цена: 0€.
