...
- [~] **4.3 Live transcription (foundation)** ⏳ — `useLiveCaptions` хук (Web Speech API, mk-MK, continuous + auto-restart, interim+final transcripts) преку `LiveCaptions` компонента; Supabase realtime broadcast на финални фрази одложен (бара DB schema); `onTranscript` callback изложен за идна интеграција
