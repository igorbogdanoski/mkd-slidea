...
import { X, Upload, FileText, Check, Loader, ChevronRight, AlertCircle } from 'lucide-react';
...
const ImportPPTXModal = ({ isOpen, onClose, onImport }) => {
  ...
  const handleFile = async (file) => {
    ...
    if (!file.name.endsWith('.pptx')) {
      setError('Само .pptx датотеки се поддржани.');
      return;
    }
    setError('');
    setLoading(true);
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-6">
          ...
          >
            ...
            <div className="flex items-center justify-between px-10 pt-10 pb-6 border-b border-slate-50">
              <div>
                <h3 className="text-2xl font-black text-slate-900">Увези PowerPoint</h3>
                <p className="text-slate-400 font-bold text-sm mt-1">Слајдовите стануваат интерактивни анкети</p>
              </div>
                ...
                >
                  ...
                  {loading ? (
                    ...
                    <>
                      ...
                      <p className="font-black text-slate-700 text-lg mb-2">Повлечи .pptx овде</p>
                      ...
                      {TYPE_OPTIONS.map(opt => (
                        <button
                          ...
                          className={`p-4 rounded-2xl border-2 text-left transition-all ${
                            pollType === opt.value ? 'border-indigo-500 bg-indigo-50' : 'border-slate-100 hover:border-slate-200'
                          }`}
                        >
                          <p className={`font-black text-sm mb-1 ${pollType === opt.value ? 'text-indigo-600' : 'text-slate-700'}`}>{opt.label}</p>
            ...
            {slides.length > 0 && (
              <div className="px-10 py-6 border-t border-slate-50 flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-3 px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-xl shadow-indigo-100 active:scale-95"
                >
                  {importing ? (
                    <><Loader size={18} className="animate-spin" /> Се увезува...</>
                  ) : (
