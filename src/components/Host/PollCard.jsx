...
import { Lock, EyeOff, RotateCcw } from 'lucide-react';

const PollCard = ({ poll, index, activePollIndex, setActivePoll }) => {
  const isActive = activePollIndex === index;
  
  return (
    <div 
      onClick={() => setActivePoll(index)} 
      className={`p-8 rounded-[2rem] border-2 cursor-pointer transition-all relative overflow-hidden ${isActive ? 'border-indigo-600 bg-indigo-50/30' : 'border-slate-50 bg-white hover:border-indigo-100'}`}
    >
      {isActive && (
