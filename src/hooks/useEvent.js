    ...
    const initializeEvent = async () => {
      try {
        ...
        for (let attempt = 0; attempt < 4; attempt++) {
          ...
          const { data, error } = await supabase
            .from('events')
            .select('id, code, title, created_at, active_poll_id, is_locked, async_mode, async_deadline, questions_moderation, brand_color, logo_url, user_id, has_password, allow_multiple_votes')
