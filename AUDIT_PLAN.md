...
## 📚 Проверка: БРО curriculum + MKD/ALB/TUR/ENG реалност (14.07.2026)

По твое појаснување дека БРО наставната програма е иста за MKD/ALB/TUR (различен јазик на настава, ист национален курикулум), а ENG е само за презентирање пред спонзори/донатори — проверив го реалниот код за да видам што точно постои наспроти што треба да се гради.

### ✅ Добра вест: curriculum tagging системот е ВЕЌЕ language-agnostic
- `polls.curriculum_tags` и `curriculum_chunks.subject/topic/grade` не содржат никаква јазична димензија — тие го таргетираат САМО предметот/темата/одделението (пр. „8-одделение-математика-разломки"), што е идентично без разлика дали часот се одржува на македонски, албански или турски. **Не треба никаква шема-промена за AL/TR учителите да го користат истиот curriculum-tagging механизам.**
- `curriculum_chunks.track` полето (gymnasium/vocational/primary/sports/music/arts) е за ТИП на образование, не јазик — исто така веќе точно.

### ⚠️ Она што реално недостасува, по слоеви
1. **AI semantic search содржина (RAG) е само на македонски.** `scripts/scrapeBroCurriculum.js` ги извлекува текстовите исклучиво од МК-јазичните БРО документи (regex-от бара „одделение", „година", „гимназија" итн., кирилица). Ако БРО објавува официјални верзии на наставните програми и на албански/турски (веројатно да, предвид уставното право на настава на овие јазици), проширувањето е чисто адитивно: нов `language` column на `curriculum_chunks` + посебен scrape run по јазик — не бара рефакторирање на постоечкото.
2. **UI преводи се многу потенки од што изгледаат.** Веќе постои `src/i18n/locales/sq.js` (албански) — но само **34 линии**, покрива исклучиво nav/footer/општи копчиња, НЕ ја покрива вистинската содржина на апликацијата (Landing.jsx маркетинг текст, Host/Dashboard/Participant текови, кои се целосно хардкодирани на македонски и воопшто не минуваат низ `t()` системот). `tr.js` (турски) и `en.js` (англиски) **воопшто не постојат**, иако `en` фигурира во SEO locale листата.
3. **Јазичното префрлување не работи преку URL.** `src/i18n/index.jsx` детектира јазик само преку `localStorage` + `navigator.language` од browser-от — `?lang=sq`/`?lang=tr` параметарот од URL-то (кој и SEO hreflang-от го очекува) воопшто не се чита. Значи дури и постоечкиот `sq.js` не може намерно да се отвори преку споделен линк денес.

### 💡 Препорака, поделено по реален приоритет
- **MKD/ALB/TUR (вистинска пазарна експанзија внатре во истата држава, не „интернационализација")**: ова е оправдана инвестиција токму затоа што не бараш нов пазар туку веќе постоечки училишен систем со уставно право на настава на трите јазици. Реален обем на работа: (а) провери дали БРО навистина објавува AL/TR верзии на документите (брзо да се провери пред да се планира scrape), (б) целосно premin на Landing.jsx и главните текови низ `t()` системот (ова е најголемиот дел работа — моментално скоро ништо не поминува низ него), (в) дополни ги `sq.js`/нов `tr.js` со целосна содржина, (г) додај `?lang=` URL читање во `i18n/index.jsx` за вистинско споделување линкови по јазик.
- **ENG за спонзори/донатори**: ова НЕ бара целосна i18n инвестиција. Бидејќи целта е презентациска (pitch), а не продукт за англиски-говоречки корисници, препорачувам засебна лесна опција — еден статичен English pitch/one-pager (или дури PDF) со клучните бројки и наративот за раст, наместо преведување на целата апликација. Многу помал обем работа, директно ја служи вистинската цел (импресионирање инвеститори/донатори), без да го отвора целото i18n прашање предвреме.

Ништо од ова не е имплементирано во оваа сесија — чисто наоди + препорака за приоритизација, бидејќи станува збор за производствена/пазарна одлука, не техничка поправка.

## 🎯 ВИСТИНСКИОТ root cause зошто фиксот од 14.07 никогаш не проработи (21.07.2026)

По твоја потврда дека вебинарот со 200+ учесници сепак не издржал и дека тоа сè уште не е тестирано, ја проверив живо `_realtime.tenants` конфигурацијата на продукцијата — и лимитите беа **вратени на оригиналните demo defaults** (`max_concurrent_users=200`), не на 1000 како што фиксот од 14.07 требаше да постигне.

### Дијагноза
- `docker-compose.yml`-от за realtime сервисот има `SEED_SELF_HOST: "true"` — ова кажува на Realtime апликацијата да **одново го засее** demo tenant-от (`realtime-dev`) со hardcoded default лимити на **секој рестарт на контејнерот**.
- Фиксот од 14.07 направи директен `UPDATE` на `_realtime.tenants`, па потоа `docker restart` „за да се преучита конфигурацијата" — но токму тој restart, поради `SEED_SELF_HOST=true`, ја **презапиша** сопствената поправка назад на defaults. Доказ: `inserted_at = updated_at` на tenant редот, точно во секундата кога контејнерот последен пат бутнал (13.07 22:46) — редот бил свежо креиран при boot, не ажуриран.
- Верификацијата од 14.07 (health check + smoke test на `/pricing`) не можела да го фати ова, бидејќи проверува дали сервисот работи, не дали конкретните лимит-вредности преживеале рестарт.

### Поправка (применета и потврдена дека преживува restart)
1. `SEED_SELF_HOST` сменето `"true"` → `"false"` во `docker-compose.yml` (бекап зачуван како `docker-compose.yml.bak-*`).
2. `docker compose up -d realtime` (recreate) — потврдено дека `updated_at` на tenant редот останува непроменет по recreate (веќе не се засејува одново).
3. Повторно применети истите целни вредности од 14.07 (`max_concurrent_users=1000`, `max_events_per_second=1000`, `max_joins_per_second=500`, `max_bytes_per_second=1000000`) — овој пат преку `supabase_admin` улогата (`postgres` нема write permission на `_realtime` шемата, за разлика од претпоставеното).
4. **Вистински тест на постојаност**: `docker restart realtime-dev.supabase-realtime` па повторна проверка — лимитите преживеаја. Ова е првиот пат конфигурацијата навистина е трајна.
5. Дополнително: PostgREST-от (`supabase-rest`) немаше експлицитен `PGRST_DB_POOL` — работеше на default од 10 конкурентни DB конекции, премногу малку за стотици истовремени гласања во бранови. Подигнато на `PGRST_DB_POOL: "30"` (Postgres `max_connections=100`, вкупна употреба во мирна состојба ~27, има простор). `max_connections` самиот не е допрен бидејќи тоа бара целосен restart на `supabase-db` (downtime за сите сервиси) — не е допрено без вистинска потреба потврдена со load test.
6. Целосен smoke test по промените: auth + REST + RPC + realtime channel subscribe — сите поминаа.

### Сепак останува нетестирано под вистински товар
Ова е конфигурациска поправка + докажана постојаност низ restart — но **сепак не е заменета со реален load test со стотици паралелни конекции**. Истата препорака од 14.07 стои, сега со уште поголема тежина бидејќи знаеме дека претходниот „фикс" никогаш реално не бил активен: пред следниот вебинар со платежен клиент, направи контролиран тест (Playwright/k6 скрипта со 200-300 паралелни participant sessions) или барем следи `docker stats`/`htop` во живо за време на следниот реален настан со >100 учесници.

## 🧪 Вистински load test — направен, резултати (21.07.2026)

Напишана Node.js скрипта (`/root/loadtest/load_test.mjs` на VPS-от) која симулира N паралелни учесници преку **истите точно** 6 realtime channel-и + `increment_vote` RPC што ги користи `useEvent.js` — не Playwright/browser-based (непотребно тешко за стотици инстанци), туку директни supabase-js клиенти, што поверно го тестира вистинскиот backend capacity.

### Важна методолошка лекција
Првите тестови (од мојата Windows машина преку домашна мрежа) покажаа катастрофални резултати на 300 учесници (~55% успех, "TypeError: fetch failed"). По длабока дијагноза (5 паралелни независни OS процеси сепак покажаа исти проблеми → значи не е single-process client library issue), заклучокот беше: **проблемот беше во МОЈАТА мрежна патека** (домашна мрежа/router/ISP), не во серверот. Потврдено со пуштање на истата скрипта **директно од VPS-от** — резултатите драстично се разликуваа. Лекција: секој идентичен load test мора да работи или директно на серверот, или од инфраструктура со гарантиран capacity (не од обичен домашен интернет), инаку резултатите се неупотребливи/лажно негативни.

### Резултати (тестирано директно од VPS-от, значи чист сигнал за вистински backend capacity)
| Учесници | Connect success | Vote success (сите гласаат ИСТОВРЕМЕНО) | Vote latency p50 / p95 |
|---|---|---|---|
| 300 | 300/300 (100%) | 300/300 (100%) | 1.6s / 2.2s |
| 500 (пред дополнителна поправка) | 500/500 (100%) | 345/500 (69%) — `statement timeout` + `ECONNRESET` | 4.9s / 6.1s |
| 500 (по поправка) | 500/500 (100%) | 427-471/500 (85-94%, варира) | 1.8s / 3.3s |

### Дополнителна поправка најдена при тестирањето
При 500 учесници се појавија реални грешки: `canceling statement due to statement timeout` (улогата `anon` има `statement_timeout=3s` — под pool queueing подолг од 3s, барањата буквално се убиваат од Postgres) и `ECONNRESET` на TLS ниво. `PGRST_DB_POOL` подигнат од 30 → **150** (Postgres `max_connections=200` сега има доволно простор) — драстично го подобри 500-учеснички резултатот (69%→85-94% успех, latency 4.9s→1.8s p50). Остатокот `ECONNRESET` на 500 (5-15%) е на TLS connection-establishment ниво (Kong/OS), не најдовме уште поточна причина — `somaxconn=4096` и Kong `worker_connections=1024` не изгледаат како ограничувачки фактор сами по себе.

### Заклучок
**300 учесници: цврсто докажано, 100% success, добра латенција.** Ова е веродостоен одговор на прашањето „дали работи со 300+" — да.
**500 учесници: работи, но со варијабилна деградација (85-94% success) под НАЈЕКСТРЕМНИОТ можен сценарио** (буквално сите гласаат во истата милисекунда — реални вебинари секогаш имаат природен jitter, luѓето не кликаат синхронизирано на милисекунда). За реална сигурност на 500, вреди дополнителна инвестиција (истражи го точниот TLS/Kong bottleneck), но 300 е веќе цврста, докажана основа.

Скриптата останува на VPS-от (`/root/loadtest/`) за идно повторно тестирање пред секој голем настан.

## 🏫 Load test — многу паралелни настани (05.08.2026)

Претходниот тест одговара на „држи ли еден настан со N учесници?". Училиште не изгледа така: една смена се дузина и повеќе паралелни часови, секој со свој настан и свои шест realtime канали, со по 20–26 ученици. Ист вкупен број врски, сосема друга форма.

Моделирано по реално училиште: 60 вработени, ~половина активни, до 26 ученици по клас, ~16 паралелни часа во смена.

| Настани × учесници | Вкупно | Распоред на гласање | Поврзување | Успех | p50 |
|---|---|---|---|---|---|
| 16 × 26 | 416 | 0ms (синтетички налет) | 100% | **100%** | 4.7–6.2s |
| 16 × 26 | 416 | 20s (реално) | 100% | **100%** | **8ms** |
| 24 × 26 | 624 | 20s | 100% | **100%** | 7ms |
| 28 × 26 | 728 | 20s | 96–99% | 53–89% (варира) | 5ms |
| 32 × 26 | 832 | 20s | 85.6% | 82.2% | 7ms |

**Што е докажано:** цела смена — 16 часа, 416 ученици — поминува без ниту еден изгубен глас, а во реалниот случај секој глас слетува за ~8ms. 24 паралелни часа со 624 ученици е сè уште 100%.

**Што НЕ е докажано:** редовите 728 и 832 не се серверски таван. VPS-от има **две јадра**, а Node процесот што го врти тестот трошеше **123% CPU** — повеќе од цело јадро — додека `supabase-db` беше на 44% на врв и најчесто под 16%. Тоа е тестот што се натпреварува со серверот за исти две јадра. Секоја бројка над ~650 од оваа поставка го мери харнесот, не системот. За повисоко: вози го товарот од втора машина во ист датацентар.

**Каде вистински би удрило прво:** при целосно синхронизиран налет (spread 0), `supabase-db` достигна 72% CPU и load average 5.5 на две јадра. Загуба на гласови сепак немаше, но латенцијата се растегна на ~6s. Тоа е сценариото за следење, а лекот се јадра — 4-јадрен сервер треба да го преполови. Realtime, PostgREST и Kong беа речиси во мирување цело време: ограничувањето е **CPU на базата**, не лимити на врски.

**Втора методолошка лекција (по онаа од 21.07):** следи го CPU-то на самиот тест. Првата лекција беше дека мрежната патека од домашна машина дава лажно негативни резултати. Оваа е нејзин близнак — тест-клиент на ист хост како серверот дава лажен таван.

Скриптите се сега во репото: `scripts/loadtest/` (`single_event_test.mjs`, `multi_event_test.mjs`, `README.md`). Ги немаат веќе вградено admin креденцијалите — се читаат од `LOADTEST_EMAIL` / `LOADTEST_PASSWORD`.
